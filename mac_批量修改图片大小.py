...
while True:
    ...
    new_img='ok_'+f
    #如果新文件夹存在则删除，不存在则创建
    if os.path.lexists(now_path + r'/' + new_img):
        shutil.rmtree(now_path + r'/' + new_img)
        new_path = os.mkdir(now_path + r'/' + new_img)
    else:
        new_path = os.mkdir(now_path + r'/' + new_img)
    new_dir = now_path + r'/' + new_img
    # 遍历文件夹下的文件，并判断是否是JPG文件
    list_file=os.listdir(now_path)
    for file_name in list_file:
        files_path = now_path + r'/' + file_name
