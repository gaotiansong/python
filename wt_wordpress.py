...
#把内容写入wordpress
def wt_wordpress(name,des,post_tag,category):
    ...
    wp = Client('http://doname.com/xmlrpc.php', 'name', 'wejoe$GEvhF$') 
...
def find_keys(key,n):
    ...
    sql="SELECT word,vo FROM shose WHERE word like '%{key}%' and vo>'{n}' ;".format(key=key,n=n)
    cur.execute(sql)
    row=cur.fetchall()
    s=[]
    for i in row:
...
while True:
    ...
    wt_wordpress(name,des,post_tag,category)
    print('成功:',n)
    n=n+1
